...
const FeaturedProjects = () => {
    ...
    useEffect(() => {
        // fetch('http://localhost:5000/works').then(res => res.json()).then(data => setWorksType(data));
        const fetchWorks = async () => {
            try {
                const url = filterType === 'all' ? 'http://localhost:5000/works' : `http://localhost:5000/works?type=${filterType}`;
